...
function processAllPromises(array) {
  function Fun(resolve, reject) {
    const arr = [];
    array.map((item) => item.then((res) => arr.push(res), (err) => arr.push(err)));
    resolve(arr);
    if (arr.length === 0) { reject('Error'); }
  }
  return new Promise(Fun);
}
...
function getFastestPromise(array) {
  const Fun = (resolve, reject) => {
    array.map((item) => item.then((result) => resolve(result), (error) => reject(error)));
  };
  return new Promise(Fun);
}
...
function chainPromises(array, action) {
  const Fun = (resolve) => {
    const arr = [];
    array.map((item) => item.then((res) => arr.push(res), (err) => err));
    setTimeout(() => { resolve(arr.reduce(action)); }, 1);
  };
  return new Promise(Fun);
}
